(function () {
  ...
  function hideModssTitles(root = document) {
    ...
    root
      .querySelectorAll(".settings-param-title")
  ...
  function removeJackett(root = document) {
    ...
    root
      .querySelectorAll('.settings-param[data-name="jackett_url2"]')
  ...
  function removeSelectboxDuplicates(root) {
    ...
    groups.forEach((group) => {
      ...
      let keeper =
        group.find((el) => el.classList.contains("selected")) || group[0];
      ...
      mutations.forEach((m) => {
        if (m.type === "childList") {
          m.addedNodes.forEach((node) => {
            ...
            const selectbox =
              node.classList && node.classList.contains("selectbox")
                ? node
  ...
  function apply() {
    ...
    document.querySelectorAll(".selectbox").forEach(removeSelectboxDuplicates);
